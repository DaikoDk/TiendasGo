...
import { MarcasApiService } from '../services/marcas.service';
...
export class MarcasFormPageComponent {
  ...
  protected readonly pendingPayload = signal<{ nombre: string; codigoMarca: string | null; activo: boolean } | null>(null);
...
private loadMarca(id: number): void {
    ...
    this.authService.validateAdminCredentials(credentials).subscribe({
      next: (session) => {
        ...
        if (this.isEditMode()) {
          ...
          this.createMarca(payload.nombre, token);
        }
      },
      error: (error: unknown) => {
        this.approvalLoading.set(false);
  ...
  private createMarca(nombre: string, token: string): void {
    this.saving.set(true);
    this.marcasApi.crearMarca(nombre, token).subscribe({
  ...
  private updateMarca(id: number, payload: { nombre: string; codigoMarca: string | null; activo: boolean }, token: string): void {
    this.saving.set(true);
