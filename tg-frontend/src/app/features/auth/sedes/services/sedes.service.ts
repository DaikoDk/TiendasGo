...
export class SedesService {
  ...
  generarEmail(nombre: string): Observable<string> {
    return this.http
      .get<unknown>(`${this.url}/generar-email`, { params: { nombre }, responseType: 'text' as 'json' })
      .pipe(map((raw) => this.unwrapGenerarEmailData(raw)));
  }

  private unwrapGenerarEmailData(raw: unknown): string {
