...
import { GerenteDropdownItem } from '../../usuarios/gerentes/models/gerente.models';
import { GerentesService } from '../../usuarios/gerentes/services/gerentes.service';
...
export class SedeFormPageComponent {
  ...
  constructor() {
    ...
    this.form.controls.nombre.valueChanges.subscribe((nombre: string | null) => {
      ...
      if (trimmed.length > 0) {
        this.sedesService.generarEmail(trimmed).subscribe((email) => this.generatedSedeEmail.set(email));
      } else {
        this.generatedSedeEmail.set('');
      }
    });
    this.generatedSedeEmail.set('');
    ...
    this.sedesService
      ...
      .subscribe({
        next: (sede) => {
          ...
          this.generatedSedeEmail.set(sede.email ?? '');
